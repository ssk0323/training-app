...
import type { CreateTrainingMenuInput, CreateTrainingRecordInput } from '../src/types'
...
// CORS middleware
app.use('*', cors({
  origin: ['http://localhost:5173', 'https://training-record-app.pages.dev'],
...
app.get('/api/records/latest/:menuId', async (c) => {
  try {
    ...
    return c.json({ 
      error: error instanceof Error ? error.message : 'Failed to fetch latest record',
      success: false 
    }, 500)
  }
})
...
app.get('/api/analytics/frequency', async (c) => {
  try {
    ...
    return c.json({ 
      error: error instanceof Error ? error.message : 'Failed to fetch frequency analytics',
      success: false 
    }, 500)
  }
})
...
app.get('/api/analytics/muscle-groups', async (c) => {
  try {
    ...
    const recentRecords = analyticsService.filterRecentRecords(allRecords, days)
    const muscleGroupStats = analyticsService.calculateMuscleGroupStats(recentRecords, allMenus)
    
    return c.json({ data: muscleGroupStats, success: true })
  } catch (error) {
    console.error('Error fetching muscle group analytics:', error)
    return c.json({ 
      error: error instanceof Error ? error.message : 'Failed to fetch muscle group analytics',
      success: false 
    }, 500)
  }
})
